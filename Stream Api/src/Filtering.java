...
List<String> locations = Arrays.asList(new String[] {"Pune","Mumbai","Chennai","Noida"});
           Stream<String> stream = locations.stream();
           List<String> result = stream.filter((location)->location.length()>5).distinct().sorted().collect(Collectors.toList());
